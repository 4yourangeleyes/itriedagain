...
type Step = 'docType' | 'contractType' | 'client';
...
export const DocumentCreationWizard: React.FC<DocumentCreationWizardProps> = ({
  ...
  const [currentStep, setCurrentStep] = useState<Step>('docType');
  ...
  const handleClientSelect = (client: Client) => {
    ...
    // Complete the flow - we have everything
    if (selectedDocType) {
      onComplete(client, selectedDocType, selectedContractType || undefined);
    }
  };
  ...
  const handleCreateNewClient = () => {
    ...
    // Complete the flow - we have everything
    if (selectedDocType) {
      onComplete(newClient, selectedDocType, selectedContractType || undefined);
    }
  };
  ...
  const handleDocTypeSelect = (docType: DocType) => {
    ...
    if (docType === DocType.CONTRACT) {
      // For contracts, ask contract type next
      setCurrentStep('contractType');
    } else {
      // For invoices, go straight to client selection
  ...
  const handleBack = () => {
    if (currentStep === 'client') {
      if (selectedDocType === DocType.CONTRACT) {
        setCurrentStep('contractType');
      } else {
        setCurrentStep('docType');
      }
      setSelectedClient(null);
    } else if (currentStep === 'contractType') {
      setCurrentStep('docType');
      ...
      <div className="bg-white border-4 border-grit-dark shadow-grit max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-gray-100 border-b-2 border-gray-300 p-4">
          ...
          <div className="flex items-center justify-center gap-4 mt-2 text-xs font-bold text-gray-600">
            <span className={selectedDocType ? 'text-green-600' : ''}>1. TYPE</span>
            {selectedDocType === DocType.CONTRACT && (
              <span className={selectedContractType ? 'text-green-600' : ''}>2. CONTRACT</span>
            )}
            <span className={selectedClient ? 'text-green-600' : ''}>{selectedDocType === DocType.CONTRACT ? '3' : '2'}. CLIENT</span>
          </div>
        </div>
        ...
        <div className="p-6">
          ...
          {/* STEP 2: Contract Type Selection (only for contracts) */}
          {currentStep === 'contractType' && (
            <div className="max-w-2xl mx-auto">
              <h3 className="text-center text-gray-600 mb-8 text-lg">
                Is this an internal HR document or external client contract?
              </h3>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Internal HR Document */}
                <button
                  onClick={() => handleContractTypeSelect(ContractType.INTERNAL_HR)}
                  className="p-8 border-4 border-gray-300 hover:border-blue-500 hover:bg-blue-50 transition-all group"
                >
                  <div className="flex flex-col items-center text-center">
                    <div className="w-20 h-20 bg-blue-100 border-4 border-blue-500 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                      <Briefcase size={40} className="text-blue-600" />
                    </div>
                    <h3 className="text-xl font-bold mb-2 group-hover:text-blue-600">Internal HR Document</h3>
                    <p className="text-sm text-gray-600 mb-4">For your team & employees</p>
                ...
                {/* External Client Document */}
                <button
                  onClick={() => handleContractTypeSelect(ContractType.EXTERNAL_CLIENT)}
                  className="p-8 border-4 border-gray-300 hover:border-purple-500 hover:bg-purple-50 transition-all group"
                >
                  <div className="flex flex-col items-center text-center">
                    <div className="w-20 h-20 bg-purple-100 border-4 border-purple-500 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                      <Handshake size={40} className="text-purple-600" />
                    </div>
                    <h3 className="text-xl font-bold mb-2 group-hover:text-purple-600">External Client Document</h3>
                    <p className="text-sm text-gray-600 mb-4">For clients & partners</p>
                    <div className="text-xs text-gray-500 text-left">
          ...
          {currentStep === 'client' && (
            <div>
              <h3 className="text-center text-gray-600 mb-6 text-lg">
                Who is this {selectedDocType === DocType.INVOICE ? 'invoice' : 'contract'} for?
              </h3>
